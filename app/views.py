...
def detail_ing2_1(request):
    ...
    member = Member.objects.get(user_id=user_id)
    contract_id = request.POST['contract_id']
    contract_all= Contract_OS.objects.filter(contract_id=contract_id)
    return render(request, 'app/detail_ing2_1.html',{'contract_all':contract_all})

def detail_ing2_2(request):
    result_dict ={}
    user_id = request.session['user_id']
    member = Member.objects.get(user_id=user_id)
    contract_id = request.POST['contract_id']
    contract_all= Contract_SR.objects.filter(contract_id=contract_id)
    return render(request, 'app/detail_ing2_2.html',{'contract_all':contract_all})

def detail_ing2_3(request):
    result_dict ={}
    user_id = request.session['user_id']
    member = Member.objects.get(user_id=user_id)
    contract_id = request.POST['contract_id']
    contract_all= Contract_SR.objects.filter(contract_id=contract_id)
    return render(request, 'app/detail_ing2_3.html',{'contract_all':contract_all})
